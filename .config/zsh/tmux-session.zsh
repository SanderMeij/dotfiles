...
tmux-session() {
    ...
    selected=$(echo "${directories[@]}" | awk '!seen[$0]++' | fzf --no-sort --query="$1" --select-1 --border=none --preview='session=$(tmux list-sessions | grep $(basename {} | tr . _)); echo "${session:-No session running, press enter to create a new session}"')
