...
tmux_session() {
    fre_directories=$(~/.cargo/bin/fre --sorted --store_name "tmux_session_history.json")
    all_directories=$(find ~/gitlab -mindepth 3 -maxdepth 3 -type d -not -path "$fre_directories")

    directories=("${fre_directories}\n~/dotfiles/\n${all_directories}")
    selected=$(echo "${directories@E}" | awk '!seen[$0]++' | fzf --no-sort --query="$1" --select-1 --border-label="Select workspace" --preview='session=$(tmux list-sessions | grep $(basename {} | tr . _)); echo "${session:-No session running, press enter to create a new session}"')
